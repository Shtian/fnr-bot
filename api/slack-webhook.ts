...
const generateBlock = ({ age, gender, fnr }: FnrInfo): SlackSectionBlock => {
    return { type: "section", text: { type: "plain_text", text: formatMsg(age, gender, fnr) } }
}
...
export default (req: NowRequest, res: NowResponse) => {
    ...
    const headerBlock: SlackSectionBlock = { type: "section", text: { type: "plain_text", text: `Her har du ${count} fødselsnummere til personer i aldere ${minAge}-${maxAge}` } }
    const blocks: SlackSectionBlock[] = [headerBlock].concat(fnrs.map(generateBlock))
